...
ENV ?= staging
# 직접 경로 지정도 가능: `make load CONFIG=load-test/config/env.staging.json`
CONFIG ?= load-test/config/env.$(ENV).json
...
help:
	...
	@echo "  make load CONFIG=load-test/config/env.staging.json"
...
smoke: check-config
	$(K6_ENV_VARS) $(K6) run $(K6_ARGS) load-test/smoke-test.js

regression: check-config
	$(K6_ENV_VARS) $(K6) run $(K6_ARGS) load-test/regression-test.js

load: check-config
	$(K6_ENV_VARS) $(K6) run $(K6_ARGS) load-test/load-test.js

stress: check-config
	$(K6_ENV_VARS) $(K6) run $(K6_ARGS) load-test/stress-test.js

spike: check-config
	$(K6_ENV_VARS) $(K6) run $(K6_ARGS) load-test/spike-test.js
